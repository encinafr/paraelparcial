...
void mostrarNumeros (ArrayList* numeros)
{
    ...
    if(numeros != NULL && numeros->isEmpty(numeros) == 0)
    {
        ...
        for (i=0; i<numeros->len(numeros); i++)
        {
            ...
            printf("%5d %-20s%-20s%-10d%d-%10d\n",j+1,getLetra(aux),getNombre(aux),getvocal(aux),getConsonante(aux),getNumero(aux));
        }
...
{
    eProducto* aux;
    FILE* archivo = fopen("Completo.csv","w");
    int i;
    int j=0;
    char comp[2] = "a";
    char comp1[2] = "e";
    ...
    if(Letras != NULL && archivo != NULL)
    {
        for(i=0; i<Letras->len(Letras); i++)
        {
            ...
            printf("%5d %-20s%-20s%-10d%-10d%-d% \n",j+1,aux->letra,getNombre(aux),aux->vocal,aux->consonante,aux->numero);
            fprintf(archivo,"%S,%s,%d,%d,%d\n",getLetra(aux),getNombre(aux),getvocal(aux),getConsonante(aux),getNumero(aux));
...
ArrayList* esVocal2(ArrayList* Letras)
{
    eProducto* aux;
    int i;
    int j=0;
    ...
    if(Letras != NULL)
    {
        for(i=0; i<Letras->len(Letras); i++)
        {
            ...
            if(strcmp(aux->letra,comp2)){
                aux->numero=0;
                aux->consonante=0;
            }
            Letras=quitarRepetidos(Letras);
...
ArrayList* depurarLista (ArrayList* repetidos, char cadena[])
{
    ...
    if(repetidos != NULL)
    {
        for(i=0; i<repetidos->len(repetidos); i++)
        {
            ...
            {
                for(s=0; s<strlen(numeroA->nombre); s++)
                {
                    if(cadena[j] == numeroA->nombre[s])
                    {
...
int setNumero(eProducto* Producto, int vocal)
{
    int aux = -1;
...
int getNumero(eProducto* Producto)
{
    int aux;
